...
const crush = (input, size) => {
  console.log(size);
    const items = Array.apply(null, Array(1000000)).map(function() { return 0 });
    console.log(items);
...
describe('crush', () => {
  test('hackerrank testing suit', (done) => {
    readFile('../hackerRank/algorithms/fixtures/crush-input.txt')
      .then((data) => {
        const input = data.split('\n');
        const [size, n] = input.shift().split(' ');
        // console.log(input);
        console.log(size);
        const biggestNumber = crush(input, size);
        done();
      });
  });
});
